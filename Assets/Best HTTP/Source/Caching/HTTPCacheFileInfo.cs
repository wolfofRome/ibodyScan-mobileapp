...
namespace BestHTTP.Caching
{
    ...
    public class HTTPCacheFileInfo : IComparable<HTTPCacheFileInfo>
    {
        ...
        public bool WillExpireInTheFuture(bool isInError)
        {
            ...
            // no-cache will always hit the network as it has to revalidate with the server before it can release the browser窶冱 cached copy (unless the server responds with a fresher response),
            // but if the server responds favourably, the network transfer is only a file窶冱 headers: the body can be grabbed from cache rather than redownloaded.
            if (this.NoCache)
                return false;
