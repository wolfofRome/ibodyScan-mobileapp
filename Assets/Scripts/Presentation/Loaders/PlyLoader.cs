...
namespace Amatib.ObjViewer.Presentation.Loaders
{
    ...
    /// PLYファイルローダー
    ///
    /// 有志が開発したRuntime版PlyImporterを改良して利用
    /// 
    /// How to load a PLY via script?（Pcx Issues）
    ///  遅く、メモリを大量に使用する（開発者談）
    /// https://github.com/keijiro/Pcx/issues/46
    /// 有志が開発したRuntime版PlyImporter
    ...
    public static class PlyLoader
    {
        ...
        public static Mesh LoadAsMesh(Stream stream, Vector3 scale)
        {
            try
            {
                var header = ReadDataHeader(new StreamReader(stream));
                var body = ReadDataBody(header, new BinaryReader(stream), scale);
        ...
        //public static async UniTask<Mesh> LoadAsMeshAsync(Stream stream)
        //{
        //    try
        //    {
        ...
        static DataBody ReadDataBody(DataHeader header, BinaryReader reader, Vector3 scale)
        {
            ...
            for (var i = 0; i < header.vertexCount; i++)
            {
                ...
                data.AddPoint(x * scale.x, y * scale.y, z * scale.z);
                data.AddColor(r, g, b, a);
            }
